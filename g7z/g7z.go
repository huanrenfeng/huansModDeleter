// custom version of https://github.com/kjk/lzmadec
package g7z
...
var (
	...
	The7zPath			string = "7z"
	the7zPath1			string = `C:\Program Files\7-Zip\7z.exe`
	the7zPath2			string = `C:\Program Files (x86)\7-Zip\7z.exe`
)
...
func (e Entry) IsDirectory() bool{
	return strings.Contains(e.Attributes,"D")
}
...
func Detect7z() error {
	...
	if detectionStateOf7z == 0 {
		if _, err := exec.LookPath(The7zPath); err == nil {
			detectionStateOf7z = 1
			
		}else if _, err := exec.LookPath(the7zPath1); err == nil{
			detectionStateOf7z = 1
			The7zPath = the7zPath1
			
		}else if _, err := exec.LookPath(the7zPath2); err == nil{
			detectionStateOf7z = 1
			The7zPath = the7zPath2
...
func newArchive(path string, password *string) (*Archive, error) {
	...
	cmd := exec.Command(The7zPath, params...)
	out, err := cmd.Output()
	if err != nil {
		return nil, err
	}
